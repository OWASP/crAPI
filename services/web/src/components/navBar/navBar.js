 ...
 */
const Navbar = props => {
  const { history, logOutUser, isLoggedIn, name, profilePicData, id } = props
    ...
    <Header>
      ...
      {isLoggedIn ? (
        <Space className='top-nav-right'>
          ...
          <Avatar size='large' src={`/identity/api/v2/avatar/${id}`} icon={<UserOutlined />} onClick={() => history.push('/my-profile')} />
        ...
        <>
          <Space className='top-nav-right'>
            ...
            <Button className='navbar-button' onClick={() => history.push('/signup')}>
              Signup
            </Button>
          </Space>
        </>
...
const mapStateToProps = ({ userReducer: { accessToken, name, isLoggedIn, id }, profileReducer: { profilePicData } }) => ({
  accessToken,
  name,
  isLoggedIn,
  profilePicData,
